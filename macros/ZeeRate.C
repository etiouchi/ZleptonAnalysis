void ZeeRate(void){
	...
	TFile *file0 = new TFile("../output/Zee_DATA.root");
	TH1D *hData = (TH1D *) file0->Get("Zee_rate");

	TH1D *hZjets = new TH1D("", "", 11, 0, 11);
	for(int i =  0; i <  12 ; i++){  //Z + light jets bgd
		TFile *file = new TFile(Form("../output/Zee_MC_%d.root",mc_RunNumber[i]));
		TH1D *h = (TH1D *) file->Get("Zee_rate");
		hZjets->Add(h);
		delete h;
		delete file;
	}
...
*/
	...
	const Float_t LumiPeriod[] = {
		14.61, 167.55,
		48.82, 142.63, 539.71, 262.15,
		391.79, 227.98, 610.04,
		1457.76, 1046.82
	};
	...
	hData->SetYTitle("Z#rightarrow ee candidats / Luminosity [pb]");
...
hZjets->Draw();
	...
	TLatex l2;
	l2.SetNDC();
	l2.DrawLatex(0.65, 0.8, "#int L dt  #approx 4.9 fb^{-1}");

	c->SaveAs("2011_ZeeRate.png");
